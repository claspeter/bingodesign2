...
async function openBuyModal(drawId) {
  ...
  // Fetch fresh balance so admin top-ups are reflected immediately
  const { ok: meOk, data: meData } = await apiFetch('/api/user-portal/me');
  if (meOk && meData) {
    currentUser.points = meData.points ?? currentUser.points;
    renderTopBar();
  }
...
function flatToCard(numbers) {
  const sorted = [...numbers].sort((a, b) => a - b);
  const groups = [sorted.slice(0,5), sorted.slice(5,10), sorted.slice(10,15)];
  return { row1: toNineCols(groups[0]), row2: toNineCols(groups[1]), row3: toNineCols(groups[2]), code: 'T' };
}

function toNineCols(fiveNums) {
  const row = Array(9).fill(null);
  fiveNums.forEach(n => {
    let c = n === 90 ? 8 : Math.min(8, Math.floor((n - 1) / 10));
    while (c < 9 && row[c] !== null) c++;
    if (c < 9) row[c] = n;
  });
  return row;
}
...
$('btnEnterRoom').addEventListener('click', async () => {
  const btn = $('btnEnterRoom');
  btn.disabled = true;
  btn.textContent = 'Loading…';

  const { ok, data } = await apiFetch('/api/user-portal/tickets');
  btn.disabled = false;
  btn.textContent = 'Enter Bingo Room →';

  if (!ok || !data.length) {
    alert('No tickets found. Buy tickets for an upcoming draw first.');
    return;
  }
  ...
  const cards = drawTickets.flatMap(t => {
    const nums = typeof t.numbers === 'string' ? JSON.parse(t.numbers) : t.numbers;
    if (Array.isArray(nums) && nums.length && typeof nums[0] === 'object' && nums[0].row1) return nums;
    return [flatToCard(Array.isArray(nums) ? nums : [])];
  });

  sessionStorage.setItem('bingoRoomTicket', JSON.stringify({
    cards,
    drawTitle: drawTickets[0].draw_title || 'Bingo Draw'
  }));

  window.location.href = '/bingo-room?automark=1';
});
